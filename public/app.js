...
SUGGESTION_COOLDOWN: 'crump_suggestion_cooldown'
        ...
        let currentFiles = [];   // Array for multiple images
        let userMemory = {
            preferences: {},
            contexts: {},
            notes: []
...
let lastErrorTime = 0;
let errorCooldownMs = 30000; // 30 seconds
...
let preferredImageGenerator = 'pollinations'; // 'pollinations' or 'segmind'
...
class ContextAwarenessEngine {
    ...
    trackMessage(role, content) {
        ...
        // Auto-detect and suggest topics
    ...
    generateInsight() {
        ...
        // Show insight after every 5 messages in a context
...
class AutonomousMessageEngine {
    constructor() {
        ...
        this.intervalPreset = 'balanced'; // Default
        this.updateIntervals();
        this.checkInterval = null;
    }
    
    updateIntervals() {
        // Set intervals based on user preference
        const presets = {
   ...
   checkForAutonomousMessage() {
    // Block autonomous messages during error state
    ...
    // Check if we should show a feature tip instead (20% chance)
...
class LearningEngine {
    constructor() {
        this.corrections = [];
        this.preferences = {
            responseLength: 'adaptive', // brief, detailed, adaptive
            codeStyle: 'modern',
            tone: 'direct',
    ...
    recordFeedback(messageId, feedbackType) {
        // feedbackType: 'thumbsUp' or 'thumbsDown'
    ...
    calculateConfidence(context, hasSearched = false) {
        ...
        // Increase confidence for topics we've learned about
        const relevantCorrections = this.getRelevantCorrections(context);
        if (relevantCorrections.length > 0) {
            confidence += 15; // We've learned from mistakes here
        }
        
        // Increase confidence if we searched
        if (hasSearched) {
            confidence += 20;
        }
        
        // Increase based on positive feedback history
        if (this.metrics.totalInteractions > 0) {
   ...
   identifyKnowledgeGaps() {
        // Analyze corrections to find patterns of weakness
        const topicCounts = {};
        ...
        // Topics with 2+ corrections are knowledge gaps
    ...
    endTrainingSession(session) {
        ...
        // Record as a learned topic if accuracy is good
        if (accuracy >= 0.7) {
    ...
    shouldRequestTraining() {
        // Determine if Crump should proactively ask for training
        ...
        // If 3+ knowledge gaps OR 30%+ negative feedback
    ...
    learnCodingStyle(codeSnippet) {
        // Analyze code to learn user's style
        const style = {
    ...
    getLearningHistory() {
        // Generate timeline of learning events
        const history = [];
        
        // Add correction milestones
        if (this.corrections.length > 0) {
    ...
    verifyKnowledge(topic) {
        // Check if knowledge on topic is solid
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    // ==========================================
    // FIX #1: RESET AUTONOMOUS TIMER ON TYPING
    // ==========================================
    const userInputField = document.getElementById('userInput');
    if (userInputField) {
        userInputField.addEventListener('input', () => {
            lastUserActivity = Date.now();
            // console.log('🔄 Activity timer reset (typing)');
        });
        
        userInputField.addEventListener('focus', () => {
            lastUserActivity = Date.now();
            // console.log('🔄 Activity timer reset (focus)');
        });
    }
   ...
   loadSettings();
    ...
    // Show continue banner if returning user with contexts
...
// ==========================================
// 🔥 ENHANCED: SMART WELCOME SYSTEM WITH TIME AWARENESS
// ==========================================
...
function addMessage(role, content, imageUrl = null, fileData = null) {
   ...
   // Track last assistant message for learning
    if (role === 'assistant') {
       ...
       // Add confidence indicator if enabled (but NOT on errors)
       ...
       // Add meta-commentary if enabled (but NOT on errors)
    if (metaCommentaryEnabled && learningEngine && !isError && content.length > 100) {            setTimeout(() => {
                const confidence = learningEngine.calculateConfidence('general', false);
...
function addMetaCommentary(messageIndex, commentary) {
    ...
    // Remove existing meta-commentary if present
...
function addConfidenceIndicator(messageIndex, confidence) {
    ...
    // Remove existing confidence indicator if present
...
async function regenerateResponse(index) {
    ...
    try {
       ...
       addMessage('assistant', data.response);
        ...
        // Manual unlock on success
        isSending = false;
        input.disabled = false;
        if (sendBtn) sendBtn.disabled = false;
        console.log('🔓 UNLOCKED (success)');

         // Check if Crump should proactively request training
        if (learningEngine && Math.random() < 0.15) { // 15% chance to check
...
// ==========================================
// 🔥 ENHANCED: SEND MESSAGE WITH AUTONOMOUS SUGGESTIONS
// ==========================================
...
async function sendMessage() {
    ...
    // Track activity for autonomous messages
    lastUserActivity = Date.now();
   ...
   if (!message && currentFiles.length === 0) {
    ...
    try {
        ...
        // Check if user is correcting Crump
        if (lastAssistantMessage && learningEngine) {
           ...
           // Detect preference changes
            ...
            // Learn coding style from code snippets
...
removeFile();
       ...
       showThinking();
        ...
        // Backup context before API call (in case of failure)
        const contextBackup = {
     ...
     if (!response.ok) throw new Error('API request failed');
       ...
       addMessage('assistant', data.response);
        ...
        if (suggestionEngine && contextsuggestionsEnabled) {
         ...
         // Check if Crump should proactively request training
        if (learningEngine && canExitErrorState() && Math.random() < 0.15) { // 15% chance to check
            const trainingRequest = learningEngine.getProactiveTrainingRequest();
            if (trainingRequest) {
                setTimeout(() => {
                    addMessage('assistant', trainingRequest);
        ...
        // Restore context from backup if corruption occurred
...
function shouldSearchWeb(message) {
    ...
    // Too short - likely conversational, don't search
    ...
    // Has time indicator AND factual question
...
function shouldGenerateImage(message) {
    ...
    return imageKeywords.some(k => lower.includes(k)) && imageNouns.some(n => lower.includes(n));
        ...
        // Second image generator using Segmind API
async function generateImageWithSegmind(prompt) {
    const encodedPrompt = encodeURIComponent(prompt);
    const timestamp = Date.now();
    
    // Segmind's free tier API endpoint
...
async function handleImageGeneration(message, retryCount = 0) {
    ...
    try {
        ...
        img.onerror = () => {
            // Try alternate generator as fallback
...
function handleFileSelect(event) {
    ...
    files.forEach((file, index) => {
        ...
        reader.onload = (e) => {
            ...
            // When all files processed, update preview
...
function shouldShowFeatureTip() {
    ...
    // Show feature tip 20% of the time during autonomous checks
    return Math.random() < 0.2;
}

// ==========================================
// AUTONOMOUS MESSAGES CONTROL
// ==========================================  
...
function loadSettings() {
   ...
   document.getElementById('voiceToggle').checked = voiceOutput;
    ...
    // Set autonomous interval preference
    ...
    // Set image generator preference
...
function toggleVoiceOutput() {
    ...
    showNotification(`✓ Autonomous messages: ${labels[interval]}`, 'success');
        ...
        // ==========================================
// LEARNING ENGINE FEEDBACK FUNCTIONS
// ==========================================
...
<div style="background: var(--bg-secondary); border: 2px solid var(--accent-primary); border-radius: 16px; padding: 24px; max-width: 600px;">
    ...
    // Add dashboard as a special message
    const chat = chats.find(c => c.id === currentChatId);
    if (chat) {
        const container = document.getElementById('chatContainer');
        container.insertAdjacentHTML('beforeend', `
    ...
    if (showConfidence) {
        ...
        // Remove all existing confidence indicators
    ...
    if (metaCommentaryEnabled) {
        ...
        // Remove all existing meta-commentary
