...
def test_parallel_runtime_asset_is_versioned_for_web_pwa_and_native():
    ...
    asset = "/runtime-body-v1.js?v=5.9.76-code-lazy-load-1"
    assert asset in shell
    assert asset in worker
    assert "ask-crump-new-body-v1-r229" in worker
    assert "ask-crump-new-body-v1-r229" in checker
...
def test_runtime_fetch_fixture_is_credential_free_and_measures_the_full_plan():
    ...
    assert "evidence.styleCount, 20" in verifier
    assert "evidence.maxStyles, 20" in verifier
