...
def test_ios_cloud_verification_cannot_sign_or_upload():
    ...
    assert 'DEVELOPER_DIR: /Applications/Xcode_26.3.app/Contents/Developer' in workflow
    assert '[[ ! -d "$DEVELOPER_DIR" ]]' in workflow
    ...
    assert workflow.index('Require Xcode 26 and iOS 26 SDK or newer') < workflow.index('npm run store:prepare:ios')
    for source in (
        'public/**',
