...
def test_image_creation_takes_precedence_over_plan_review_after_authentication():
    ...
    assert "if (creationKind !== 'image') dispatchPendingPlanIntent();" in precedence
    assert "if (intent.kind === 'image') discardPendingPlanIntent();" in controller
    assert "if (creationKind === 'image') discardPendingPlanIntent();" not in precedence
    assert "url.searchParams.delete('intent')" in controller
    assert "url.searchParams.delete('plan')" in controller
    assert "url.searchParams.delete('signup')" in controller
    assert "url.searchParams.delete('source')" in controller
...
def test_image_auth_handoff_has_same_tab_cross_device_and_fail_closed_browser_proof():
    ...
    assert "signup=1&source=image&plan=professional&intent=image" in verifier
    assert "verification=success&intent=private-prompt" in verifier
    assert "verification=success&intent=image&plan=professional&imageRuntime=0" in verifier
    ...
    assert "verifyUnavailableImageStudio" in verifier
    assert "verifyAuthenticatedFullCta" in verifier
    assert "verifyExistingPresentationPlan" in verifier
    assert "page.on('request'" in verifier
    assert "method !== 'GET'" in verifier
    assert "pathname.startsWith('/api/')" in verifier
    assert "networkViolations" in verifier
    assert "assert.deepEqual(current.networkViolations, [])" in verifier
    assert "JSON.parse(result.pendingPlan).plan, 'professional'" in verifier
    assert "delivered.registration.visible, false" in verifier
    assert "replay.registration.visible, false" in verifier
    assert "assert.deepEqual(recovered.calls, [{tool: 'image', action: 'open'}])" in verifier
    assert "return {blocked: result, recovered};" in verifier
    for boundary in ("media", "features", "billing", "provider", "checkout", "upload", "generation", "credit"):
