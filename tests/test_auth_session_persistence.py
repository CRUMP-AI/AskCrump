...
def test_same_page_login_is_serialized_and_confirmed():
    ...
    assert 'src="/device-auth.js?v=5.9.54"' in shell
    assert "'/device-auth.js?v=5.9.54'" in worker
...
def test_sleeping_pwa_checks_for_updates_and_adopts_safe_signed_out_runtime():
    ...
    assert 'src="/install-prompt.js?v=5.9.54"' in shell
    assert "'/install-prompt.js?v=5.9.54'" in worker
