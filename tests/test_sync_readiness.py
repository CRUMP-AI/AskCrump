...
def test_changed_sync_manager_is_release_versioned_and_network_first():
    ...
    assert '<script defer src="/sync-manager.js?v=5.9.60"></script>' in shell
    assert "'/sync-manager.js?v=5.9.60'" in worker
    assert '<script defer src="/presence-manager.js?v=5.9.60"></script>' in shell
    assert "'/presence-manager.js?v=5.9.60'" in worker
    assert '<script defer src="/chat-sync.js?v=5.9.60"></script>' in shell
    assert "'/chat-sync.js?v=5.9.60'" in worker
