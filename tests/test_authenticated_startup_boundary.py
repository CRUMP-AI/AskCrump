...
def test_protected_product_modules_wait_for_authenticated_workspace():
    ...
    assert 'if (!window.currentUser) return;' in code
    assert "window.addEventListener('crump:authenticated-ready', hydrateAuthenticatedAvailability)" in code
...
def test_signed_out_initializers_do_not_directly_hydrate_protected_data():
    ...
    code_boot = code[code.index("  if (document.readyState === 'loading')"):]
    assert 'void refreshAvailability();' not in code_boot
    assert 'hydrateAuthenticatedAvailability();' in code_boot
