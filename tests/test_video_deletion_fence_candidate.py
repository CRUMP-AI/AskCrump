...
async def test_existing_cron_reconciles_deleted_video_before_other_workers(monkeypatch):
    class AccountDeletionWorker:
        async def process_due(self, *, limit):
            assert limit == 2
            return {"processed": 0, "deleted": 0, "completed": 0, "retrying": 0}
    ...
    monkeypatch.setattr(manuscript_routes, "account_deletions", AccountDeletionWorker())
    ...
    assert response == {
        ...
        "accountDeletions": {
            "processed": 0, "deleted": 0, "completed": 0, "retrying": 0,
        },
...
def test_sql_recovery_uses_atomic_exact_token_locks_and_service_only_rpcs():
    sql = (ROOT / "staging/video_provider_deletion_fence.sql").read_text(
        encoding="utf-8"
    ).lower()
    ...
    establish = function_body("establish_account_deletion_fence")
    recover = function_body("recover_video_account_deletion_fence")
    begin = function_body("begin_video_account_deletion")
    for body in (establish, recover):
        assert body.index("from public.users") < body.index(
            "from public.account_deletion_jobs"
        )
        assert body.index("from public.account_deletion_jobs") < body.index(
            "from public.video_account_deletion_fences"
        )
        assert body.index("from public.video_account_deletion_fences") < body.index(
            "from public.video_provider_start_claims"
        )
    assert begin.index("from public.users") < begin.index(
        "from public.account_deletion_jobs"
    )
    assert begin.index("select * into deletion_fence") < begin.index(
        "update public.video_provider_start_claims"
    )
    assert "deletion_fence.operation_token <> p_operation_token" in recover
    assert "deletion_fence.requested_at > now() - interval '15 minutes'" in recover
    assert "deletion_job.created_at > now()" not in recover
    assert "if not fence_found then" in recover
    assert "p_require_stale boolean default false" in sql
    assert (
        "revoke all on function public.recover_video_account_deletion_fence"
        "(uuid, uuid, boolean)" in sql
    )
    assert (
        "grant execute on function public.recover_video_account_deletion_fence"
        "(uuid, uuid, boolean) to service_role" in sql
    )
