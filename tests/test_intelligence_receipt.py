...
def test_intelligence_receipt_assets_are_cache_versioned_atomically():
    ...
    image_node_version = "5.9.76-outcome-refinement-recovery-1"
    loader_version = "5.9.76-conversation-action-labels-1"
    ...
    for asset, version in (
        ...
        ("ui-functions.js", image_node_version),
    ):
        versioned = f"/{asset}?v={version}"
        assert versioned in runtime
        assert versioned in worker
    for source in (runtime, worker):
        assert "/app.js?v=5.9.76-settings-save-isolation-1" in source
    assert "ask-crump-new-body-v1-r232" in worker
