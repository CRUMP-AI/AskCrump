...
def test_marketing_ctas_are_first_party_analytics_events():
    ...
    assert '<script defer src="/landing.js?v=5.9.53"></script>' in page
    assert '<link rel="stylesheet" href="/landing-5.6.css?v=5.9.53">' in page
...
def test_use_case_pages_are_unique_crawlable_and_attribution_ready():
    ...
    for page, title, slug, source, file_type in expectations:
        ...
        assert '<script defer src="/landing.js?v=5.9.53"></script>' in page
        assert '<link rel="stylesheet" href="/landing-5.6.css?v=5.9.53">' in page
        assert '<link rel="stylesheet" href="/use-case.css?v=5.9.53">' in page
        assert '/_vercel/insights/script.js' in page
...
def test_release_version_and_cache_advance_together():
    ...
    assert '"version": "5.9.53"' in package
    assert "__version__ = '5.9.53'" in backend
    assert "ask-crump-new-body-v1-r87" in worker
    assert "/landing-5.6.css?v=5.9.53" in worker
    assert "/use-case.css?v=5.9.53" in worker
    assert "/landing.js?v=5.9.53" in worker
...
def test_changed_activation_assets_are_release_versioned():
    ...
    for asset in (
        "/conversation.css?v=5.9.53",
        "/crump-v1-body.css?v=5.9.53",
        "/ui-functions.js?v=5.9.53",
        "/device-auth.js?v=5.9.53",
        "/product-analytics.js?v=5.9.53",
        "/auth-controller.js?v=5.9.53",
        "/app.js?v=5.9.53",
    ):
    ...
    runtime = read("public/runtime-body-v1.js")
    assert "/crump-4.3.js?v=5.9.53" in runtime
    assert "/crump-4.3.js?v=5.9.53" in worker
