...
@pytest.mark.asyncio
async def test_image_request_retries_one_transient_provider_failure(monkeypatch):
    calls = 0

    async def handler(request: httpx.Request) -> httpx.Response:
        nonlocal calls
        calls += 1
        return httpx.Response(503 if calls == 1 else 200, request=request, json={'data': []})
