-- Ask Crump growth operating snapshot.
-- Returns aggregate funnel and retention counts only; never identifiers or user content.

begin;

create index if not exists users_growth_cohort_idx
  on public.users(created_at, id)
  where deleted_at is null;

create index if not exists product_events_growth_journey_idx
  on public.product_events(user_id, environment, event_name, created_at);
...
begin
  ...
  cross join lateral (
    values
      ...
      (8::smallint, 'durable_value_reached'::text, aha_accounts, cohort_accounts),
      (9::smallint, 'response_shared'::text, shared_accounts, cohort_accounts),
      (10::smallint, 'plan_intent_reached'::text, plan_intent_accounts, cohort_accounts),
      (11::smallint, 'checkout_opened'::text, checkout_opened_accounts, cohort_accounts),
      (12::smallint, 'checkout_completed'::text, checkout_completed_accounts, cohort_accounts),
      (13::smallint, 'active_paid_now'::text, active_paid_accounts, cohort_accounts),
      (14::smallint, 'd1_returned'::text, d1_returned_accounts, d1_eligible_accounts),
      (15::smallint, 'd7_returned'::text, d7_returned_accounts, d7_eligible_accounts)
...
comment on function public.product_growth_funnel_snapshot(
  ...
  'Service-role-only aggregate growth funnel. Returns counts and rates, never account identifiers or user content.';
