-- Ask Crump 5.9.21
-- Keep pre-instrumentation accounts out of comparable product-event cohorts.
...
comment on function public.product_growth_funnel_snapshot(
  ...
  'Service-role-only aggregate growth funnel for accounts created after both the requested window start and the first observed production event boundary (2026-08-23 09:10:55.602863+00). Includes recent-work continuation and binary outcome feedback; returns counts and rates, never account identifiers or user content.';
