...
def normalize_repo_source(url: Any, revision: Any = None) -> tuple[str, str | None]:
    ...
    if (
        ...
        raise ValueError(
            "Autonomous Crump currently accepts public https://github.com repositories only."
        )
...
class CodeTaskService:
        ...
        *,
        ...
        if target not in TRANSITIONS.get(current, frozenset()):
            raise CodeTaskConflictError(
                f"Autonomous Crump task cannot move from {current} to {target}."
            )
        ...
        if not rows:
            raise CodeTaskConflictError(
                "Autonomous Crump task changed while this request was running."
            )
        updated = rows[0]
        if event_type:
    ...
    async def requeue_after_failure(
        ...
        *,
        ...
        if not rows:
            raise CodeTaskConflictError(
                "Autonomous Crump task changed before its retry was saved."
            )
        updated = rows[0]
    ...
    async def mark_refunded(self, task: dict[str, Any]) -> dict[str, Any]:
        ...
        if not rows:
            raise CodeTaskConflictError(
                "Autonomous Crump refund state changed while settling."
            )
        return rows[0]
