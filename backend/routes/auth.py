...
async def register(payload: RegisterRequest, request: Request):
    ...
    await enforce_auth_rate_limit(db, request, action='register', identity=email, identity_limit=5, ip_limit=20, window_seconds=3600)
    if not validate_email(email):
        return JSONResponse(status_code=400, content={'success': False, 'error': 'Enter a valid email address.'})
    ...
    if existing and existing.get('is_verified'):
        return JSONResponse(status_code=409, content={'success': False, 'error': 'An account with that email already exists.'})
    ...
    if existing:
        ...
        await db.upsert('user_settings', {'user_id': user['id'], 'updated_at': now}, on_conflict='user_id')

    sent = await email_service.send_verification(email, user.get('full_name'), verification_token)
    if sent:
        message = 'Verification email resent. Check your inbox.' if pending_account else 'Account created. Check your email to verify it.'
    else:
...
async def login(payload: LoginRequest, request: Request, response: Response):
    email = normalize_email(str(payload.email))
    await enforce_auth_rate_limit(db, request, action='login', identity=email, identity_limit=15, ip_limit=60, window_seconds=900)
    ...
    if not user.get('is_verified'):
        return JSONResponse(status_code=403, content={'success': False, 'error': 'Verify your email before signing in.', 'needsVerification': True, 'email': email})

    raw_token, session = await create_session(
        db, settings, user, request,
        device_name=payload.deviceName,
        platform=payload.platform,
    )
    await db.update('users', {'last_login': iso_now(), 'updated_at': iso_now()}, filters={'id': eq(user['id'])})
    set_session_cookie(response, raw_token, request)
    user_settings = await db.select_one('user_settings', filters={'user_id': eq(user['id'])})
    return {
        'success': True,
...
async def check_session(request: Request, response: Response):
    ...
    user_settings = await db.select_one('user_settings', filters={'user_id': eq(auth.user['id'])})
    return {
        'success': True,
        'authenticated': True,
        'data': {
...
async def refresh_session(request: Request, response: Response):
    auth = await authenticate_request(request, db, settings)
    set_session_cookie(response, auth.token, request)
    return {'success': True, 'user': public_user(auth.user), 'expiresAt': auth.session.get('expires_at')}
...
async def logout(request: Request, response: Response):
    try:
        ...
        device_id = str(auth.session.get('device_id') or request.headers.get('x-installation-id') or '')[:200]
        await db.update('sessions', {'revoked_at': iso_now()}, filters={'id': eq(auth.session['id']), 'user_id': eq(auth.user['id'])})
        if device_id:
            await db.update('push_tokens', {'enabled': False, 'updated_at': iso_now()}, filters={'user_id': eq(auth.user['id']), 'installation_id': eq(device_id)})
    except AuthenticationError:
...
async def logout_all(request: Request, response: Response):
    auth = await authenticate_request(request, db, settings)
    now = iso_now()
    await db.update('sessions', {'revoked_at': now}, filters={'user_id': eq(auth.user['id']), 'revoked_at': 'is.null'})
    await db.update('push_tokens', {'enabled': False, 'updated_at': now}, filters={'user_id': eq(auth.user['id']), 'enabled': eq(True)})
    clear_session_cookie(response)
...
async def list_devices(request: Request):
    ...
    sessions = await db.select(
        ...
        filters={'user_id': eq(auth.user['id']), 'revoked_at': 'is.null', 'expires_at': gt(iso_now())},
...
async def revoke_device(payload: RevokeDeviceRequest, request: Request):
    auth = await authenticate_request(request, db, settings)
    session = await db.select_one('sessions', columns='id,device_id', filters={'id': eq(payload.sessionId), 'user_id': eq(auth.user['id'])})
    now = iso_now()
    await db.update('sessions', {'revoked_at': now}, filters={'id': eq(payload.sessionId), 'user_id': eq(auth.user['id'])})
    device_id = str((session or {}).get('device_id') or '')
    if device_id:
        await db.update('push_tokens', {'enabled': False, 'updated_at': now}, filters={'user_id': eq(auth.user['id']), 'installation_id': eq(device_id)})
    return {'success': True}
...
async def forgot_password(payload: EmailRequest, request: Request):
    email = normalize_email(str(payload.email))
    await enforce_auth_rate_limit(db, request, action='forgot', identity=email, identity_limit=5, ip_limit=20, window_seconds=3600)
    user = await db.select_one('users', columns='id,email,full_name', filters={'email': eq(email)})
...
async def reset_password(payload: ResetPasswordRequest, request: Request):
    await enforce_auth_rate_limit(db, request, action='reset', identity=token_hash(payload.token), identity_limit=10, ip_limit=30, window_seconds=3600)
    password_ok, password_error = validate_password(payload.newPassword)
    if not password_ok:
        return JSONResponse(status_code=400, content={'success': False, 'error': password_error})
    user = await db.select_one('users', columns='*', filters={
        'password_reset_token_hash': eq(token_hash(payload.token)),
        'password_reset_expires': gt(iso_now()),
    })
    if not user:
        return JSONResponse(status_code=400, content={'success': False, 'error': 'This reset link is invalid or expired.'})
    await db.update('users', {
    ...
    await db.update('sessions', {'revoked_at': iso_now()}, filters={'user_id': eq(user['id']), 'revoked_at': 'is.null'})
...
async def resend_verification(payload: EmailRequest, request: Request):
    email = normalize_email(str(payload.email))
    await enforce_auth_rate_limit(db, request, action='resend', identity=email, identity_limit=5, ip_limit=20, window_seconds=3600)
    user = await db.select_one('users', columns='*', filters={'email': eq(email)})
    if not user or user.get('is_verified'):
        return {'success': True, 'message': 'If verification is needed, a new email has been sent.'}
    raw_token = random_token(40)
    await db.update('users', {
    ...
    return {'success': True, 'message': 'Verification email sent.' if sent else 'Email delivery is not configured.'}
...
async def verify_email(token: str):
    ...
    if not user:
        return RedirectResponse(f'{settings.app_url}/app?verification=failed', status_code=303)
    if user.get('is_verified'):
        return RedirectResponse(f'{settings.app_url}/app?verification=already_verified', status_code=303)
    await db.update('users', {
        'is_verified': True,
    ...
    return RedirectResponse(f'{settings.app_url}/app?verification=success', status_code=303)
