...
- Apple: the current native candidate explicitly supports both iPhone and iPad. The release packet
  ...
  size (`2064×2752` or `2048×2732`), or the corresponding landscape dimensions. Use JPEG or 24-bit
  RGB PNG with no alpha/transparency. Do not remove the iPad set unless the signed app is first
...
- Google Play: use 4–8 phone screenshots for recommendation eligibility. Each must be 9:16 portrait
  ...
  exceed 3840 pixels. Use JPEG or 24-bit RGB PNG with no alpha/transparency, no larger than 8 MB.
