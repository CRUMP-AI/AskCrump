...
| --- | --- | --- |
| Real-user performance measurement across growth surfaces | Commit `ddb4344`; deployment `dpl_8H52xj6sQDmT55HznVmcmLenmxbo`; Vercel Speed Insights was enabled but had no events, Real Experience Score, or Core Web Vitals sample. The existing project-hosted collector now loads exactly once on the Ask Crump homepage, four acquisition pages, app shell, and Clever Crump parent page without adding a package, key, provider account, environment variable, or paid API. All seven live documents and the collector route returned 200; homepage, app, and parent-company browser checks found the exact script, no horizontal overflow, and no warning/error console entry. All 433 regressions, 44 JavaScript validations, explicit Python compilation, production preflight, native web build, and store metadata checks passed. All six aliases point to the release; its observed runtime responses were 200 with no runtime error cluster or warning/error/fatal log. Full evidence is in `docs/REAL_USER_PERFORMANCE_MEASUREMENT_RELEASE_2026-08-29.md`. | Verified instrumentation; legitimate field-performance sample pending |
