...
| --- | --- | --- |
| Real-user performance measurement across growth surfaces | Commits `ddb4344`, `a3fc449`, and `448841a`; deployments `dpl_8H52xj6sQDmT55HznVmcmLenmxbo`, `dpl_DtaaNbQUhvpvekMjG973eBfjuyGy`, and `dpl_3KmLcZ97Pra7DpG8qY7Drw5kQ1z3`; Vercel Speed Insights was enabled but had no events, Real Experience Score, or Core Web Vitals sample. The project-hosted collector now loads exactly once on the Ask Crump homepage, four acquisition pages, app shell, and Clever Crump parent page without adding a package, key, provider account, environment variable, or paid API. Explicit routes corrected seven verification events grouped as `Unknown`; a bounded follow-up produced three labeled `/` events. A subsequent audit found that `/app` accepts password-reset tokens in its query string, so a shared fail-closed `beforeSend` boundary now removes all query strings and fragments before performance transmission. An executable contract reduces a synthetic token URL to the canonical app URL; live production loads that configuration before the collector. No real token or customer content was used, and no prior real-token capture is claimed. All three passes completed the full regression, JavaScript, production, native-web, and store-metadata checks; the latest pass covered 434 regressions and 45 JavaScript files. All six aliases point to the redaction release; its observed runtime group contained 51 HTTP 200 responses with no runtime error cluster or warning/error/fatal log. Full evidence is in `docs/REAL_USER_PERFORMANCE_MEASUREMENT_RELEASE_2026-08-29.md`. | Verified route-labeled, URL-redacted instrumentation; legitimate mobile and desktop field sample pending |
