...
| --- | --- | --- |
| Real-user performance measurement across growth surfaces | Commits `ddb4344` and `a3fc449`; deployments `dpl_8H52xj6sQDmT55HznVmcmLenmxbo` and `dpl_DtaaNbQUhvpvekMjG973eBfjuyGy`; Vercel Speed Insights was enabled but had no events, Real Experience Score, or Core Web Vitals sample. The existing project-hosted collector now loads exactly once on the Ask Crump homepage, four acquisition pages, app shell, and Clever Crump parent page without adding a package, key, provider account, environment variable, or paid API. The first seven verification events arrived as `Unknown`; explicit route labels now distinguish every surface, and one bounded follow-up produced three `/` events beside the seven historical events. These ten events are verification traffic, not customer-performance evidence. All seven live documents and the collector route returned 200; homepage, app, and parent-company browser checks found no overflow or warning/error console entry. Both release passes completed all 433 regressions, 44 JavaScript validations, production preflight, native web build, and store metadata checks. All six aliases point to the repaired release; observed runtime responses were 200 with no runtime error cluster or warning/error/fatal log. Full evidence is in `docs/REAL_USER_PERFORMANCE_MEASUREMENT_RELEASE_2026-08-29.md`. | Verified labeled instrumentation; legitimate mobile and desktop field sample pending |
