...
2. Test the SQL with real PostgreSQL transactions and two connections:
   reservation/deletion lock ordering; provider acceptance vs user delete;
   a crash after dispatch; duplicate RPC response; trigger behavior through
   `delete_user_account`; lease expiry and `SKIP LOCKED`. Fakes do not prove
   database locking, grants, RLS, or trigger syntax.
...
Next action: independently review this candidate, obtain a disposable
PostgreSQL transaction test, produce a real migration via the CLI, and
approve the staged rollout or keep video deletion release-held.
