...
Exact real-PostgreSQL evidence exists for repaired executable source identity `1abd86f4a11c152881f84b69fc7a13d06edd1629`. CI-only mirror `3cbb714bae797a366b5e495937100a3462bd0c60` differs from that source only by the three-line branch trigger needed to run the otherwise manual workflow. [GitHub Actions run 35170038398](https://github.com/CRUMP-AI/AskCrump/actions/runs/35170038398) passed both caller-owned disposable PostgreSQL 15 and PostgreSQL 17 jobs. The later candidate commit that records this receipt changes only this evidence document and its assertion test; it does not alter the SQL, harness, application, or workflow bytes proven by that run.
...
1. Independently review SQL semantics, indexes, lock order, deadlock behavior, PostgREST JSON shapes, and the binding between the exact executable source identity and its CI-only mirror.
2. Re-read the fresh remote migration ledger and resolve any later migration before application.
3. Prove the complete application suite and protected CI on the eventual integration identity.
4. Prove live OIDC/Sandbox provisioning, deny-all networking, cancellation, destruction, retry, refund, monitoring, rollback, and actual provider quality/cost/latency gates.
5. Make a separate, explicit release decision; changing either false feature gate is outside this candidate.
