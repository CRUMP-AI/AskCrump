...
submission. Android source was regenerated for 5.9.26/build 50926 and its unsigned Release App
...
| Area | Evidence | State |
| --- | --- | --- |
| Production release | Production health returned 5.9.26; the deployed client contains the direct `Keep in a Project` action, two crawlable workflow pages, verified referral-copy handling, and canonical native API defaults; its deployment runtime-error scan was empty. | Verified |
...
| Android release identity | Regenerated source uses version 5.9.26 and build/version code 50926. | Verified |
...
| Android cloud boundary | `.github/workflows/android-store-verify.yml` prepares source with Node 22, selects Temurin Java 21, compiles a Release App Bundle, and requires the `.aab` to be non-empty. The refreshed 5.9.26/build 50926 [run 33124724445](https://github.com/CRUMP-AI/AskCrump/actions/runs/33124724445) passed every source, signing-control, Gradle, and bundle-output step. | Verified unsigned `.aab` compile |
