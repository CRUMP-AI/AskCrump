...
submission. Android source was regenerated for 5.9.27/build 50927 and its unsigned Release App
...
| Area | Evidence | State |
| --- | --- | --- |
| Production release | Production health returned 5.9.27; the deployed client contains the direct `Keep in a Project` action, two crawlable workflow pages, verified referral-copy handling, canonical native API defaults, and the backward-compatible credits webhook secret-key alias. Both direct live Stripe destinations returned 200 to signed harmless replays; the payment routes had no runtime error cluster. | Verified |
...
| Android release identity | Regenerated source uses version 5.9.27 and build/version code 50927. | Verified |
...
| Android cloud boundary | `.github/workflows/android-store-verify.yml` prepares source with Node 22, selects Temurin Java 21, compiles a Release App Bundle, and requires the `.aab` to be non-empty. The refreshed 5.9.27/build 50927 [run 33126121646](https://github.com/CRUMP-AI/AskCrump/actions/runs/33126121646) passed every source, signing-control, Gradle, and bundle-output step. | Verified unsigned `.aab` compile |
